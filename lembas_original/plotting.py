...
def movingaverageSTD(X, Y, window=5):
    Xmove = numpy.mean(numpy.lib.stride_tricks.sliding_window_view(X, window_shape = window), axis=1)
    Ymove = numpy.mean(numpy.lib.stride_tricks.sliding_window_view(Y, window_shape = window), axis=1)
    YmoveSTD = numpy.std(numpy.lib.stride_tricks.sliding_window_view(Y, window_shape = window), axis=1)
    return Xmove, Ymove, YmoveSTD
...
def lineOfIdentity():
    ...
    plt.text(0.9, 0.8, 'X=Y ', ha='right', va='top', transform = plt.gca().transAxes)
...
def contourPlot(X, Y, Z):
    ...
    #df = df.round(1)
    pivotted = df.pivot(values='Z_value', index='Y_value', columns='X_value')
...
def initProgressObject(maxIter):
    ...
    stats['endTime'] = 0
    stats['itertime'] = float('nan')*numpy.ones(maxIter)
...
def printTime(stats):
    deltaT = stats['endTime']- stats['startTime']
    print('Time:',  deltaT, deltaT/60, deltaT/3600)    
...
# def getDeltaT(timeVector):
#     elements = numpy.argwhere(numpy.isnan(timeVector) == False)
#     values = timeVector[elements]
#     delta = values[1:] - values[0:-1]
#     delta = numpy.insert(delta, 0)
...
def compareTFcorrelations(Yhat, Y, YtestHat, Ytest, outputNames):
    ...
    plt.scatter(resultTrain, resultTest)
    lineOfIdentity()
    plt.xlabel('Train correlation')
    plt.ylabel('Test corelation')

    r, p = pearsonr(resultTest.flatten(), resultTrain.flatten())
    plt.text(0.1, 0.9, 'r {:.2f} \np {:.2e}'.format(r, p), horizontalalignment='left',  verticalalignment='center', transform = plt.gca().transAxes)

    for i in range(len(outputNames)):
        plt.text(resultTrain[i], resultTest[i], outputNames[i])
