...
def split_data(X_in: torch.Tensor, 
    ...
    if not sum([v for v in train_split_frac.values() if v]) == 1:
        raise ValueError('Train-test-validation split must sum to 1')
    
    if not train_split_frac['validation']:
        X_train, X_test, y_train, y_test = train_test_split(X_in, 
                                                        y_out, 
                                                        test_size=train_split_frac['train'],
                                                        random_state=seed)
...
def train_signaling_model(mod,  
    ...
    # set up data objects
    train_data = ModelData(X_in.to('cpu'), y_out.to('cpu'))
    if mod.device == 'cuda':
        pin_memory = True
    else:
        pin_memory = False
...
# def train_signaling_model_depr(mod,  
#                           optimizer: torch.optim, 
#                           loss_fn: torch.nn.modules.loss,
#                           reset_epoch : int = 200,
#                           hyper_params: Dict[str, Union[int, float]] = None,
...
#         number of cores for parallelization, by default 0
#     verbose : bool, optional
#         whether to print various progress stats across training epochs
